...
RULES: list[tuple[re.Pattern, str]] = [
    ...
    # Groceries — US + India
    (re.compile(r"whole\s*foods|trader\s*joe|kroger|safeway|wegmans|publix|costco|target\s*grocery|walmart|stop\s*&?\s*shop|aldi|lidl|fresh\s*market|sprouts|food\s*lion|market\s*basket|giant\s*food|h[-\s]?e[-\s]?b|hannaford|meijer|weis|grocery\s*square|patels\s*cash|cash\s*and\s*carry|food\s*bazaar|compare\s*foods|international\s*grocery|big\s*bazaar|dmart|reliance\s*fresh|zepto|blinkit|grofer|swiggy\s*instamart|bigbasket", re.I), "Groceries"),
